// Procedural material atlas generator for VOID BASTION.
//
//   node tools/gentex.mjs           write tex/*.png
//   node tools/gentex.mjs --check   regenerate and diff against what is committed
//
// Zero dependencies. Writes two 1024x1024 atlases, each a 4x4 grid of 256px
// tiles, sixteen materials in the order listed in MATERIALS:
//
//   tex/material.png   RGB albedo detail, A ambient occlusion
//   tex/surface.png    RG tangent normal, B roughness, A metalness
//
// Everything is seeded and deterministic, so regenerating produces
// byte-identical files. That matters: the filenames carry a content hash, and
// a hash that churned on every run would break caching for no reason.
//
...
function fbm(x, y, f, octaves, seed, gain = 0.5) {
...
function fbmAniso(x, y, fx, fy, octaves, seed) {
 ...
 */
function worleyEdge(x, y, f, seed) {
  const gx = x * f, gy = y * f;
  const cx = Math.floor(gx), cy = Math.floor(gy);
  let f1 = 9, f2 = 9;
  for (let j = -1; j <= 1; j++) {
 ...
 */
function panels(x, y, nx, ny, seed) {
...
function scratches(x, y, n, seed, len = 0.5) {
  let best = 1;
  for (let i = 0; i < n; i++) {
